[**Wang Language v0.15.0**](README.md)

***

# Wang Language v0.15.0

Wang Language - A CSP-safe workflow programming language
Uses Nearley to generate a standalone parser with no runtime dependencies

## Classes

- [CircularDependencyError](classes/CircularDependencyError.md)
- [FunctionNotFoundError](classes/FunctionNotFoundError.md)
- [InMemoryModuleResolver](classes/InMemoryModuleResolver.md)
- [ModuleNotFoundError](classes/ModuleNotFoundError.md)
- [ModuleResolver](classes/ModuleResolver.md)
- [PausableWangInterpreter](classes/PausableWangInterpreter.md)
- [TypeMismatchError](classes/TypeMismatchError.md)
- [UndefinedVariableError](classes/UndefinedVariableError.md)
- [WangError](classes/WangError.md)
- [WangInterpreter](classes/WangInterpreter.md)
- [WangValidator](classes/WangValidator.md)

## Interfaces

- [CallFrame](interfaces/CallFrame.md)
- [ErrorContext](interfaces/ErrorContext.md)
- [ExecutionContext](interfaces/ExecutionContext.md)
- [ExecutionState](interfaces/ExecutionState.md)
- [InterpreterOptions](interfaces/InterpreterOptions.md)
- [ModuleResolution](interfaces/ModuleResolution.md)
- [ParserOptions](interfaces/ParserOptions.md)
- [PausableInterpreterOptions](interfaces/PausableInterpreterOptions.md)
- [SerializedContext](interfaces/SerializedContext.md)
- [SerializedState](interfaces/SerializedState.md)
- [ValidationResult](interfaces/ValidationResult.md)

## Variables

- [validator](variables/validator.md)
- [VERSION](variables/VERSION.md)

## Functions

- [createInterpreter](functions/createInterpreter.md)
