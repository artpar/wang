...
exports.WangInterpreter = void 0;
...
class WangInterpreter {
     ...
     */
    collectVariablesFromContextChain() {
        const variables = {};
        const userVariables = {};
    ...
    enhanceErrorWithContext(error, node) {
    ...
    bindBuiltins() {
        ...
        this.bindFunction('log', (...args) => {
            this.consoleLogs.push({ type: 'log', args, timestamp: Date.now() });
            console.log(...args);
        });
        this.bindFunction('error', (...args) => {
            this.consoleLogs.push({ type: 'error', args, timestamp: Date.now() });
            console.error(...args);
        });
        this.bindFunction('warn', (...args) => {
            this.consoleLogs.push({ type: 'warn', args, timestamp: Date.now() });
            console.warn(...args);
        });
        // Type conversion functions
        this.bindFunction('Number', (val) => Number(val));
        this.bindFunction('String', (val) => String(val));
        this.bindFunction('Boolean', (val) => Boolean(val));
        // Type checking functions
        this.bindFunction('isNaN', (val) => isNaN(val));
        this.bindFunction('isFinite', (val) => isFinite(val));
        this.bindFunction('isInteger', (val) => Number.isInteger(val));
        // Register all stdlib functions
        Object.entries(index_1.stdlib).forEach(([name, fn]) => {
            this.bindFunction(name, fn);
        });
        ...
        this.bindFunction('filter', async (arr, predicate) => {
            const results = [];
        ...
        this.bindFunction('map', async (arr, mapper) => {
            const results = [];
            for (let i = 0; i < arr.length; i++) {
        ...
        this.bindFunction('reduce', async (arr, reducer, initial) => {
        ...
        this.bindFunction('some', async (arr, predicate) => {
            for (let i = 0; i < arr.length; i++) {
                if (await predicate(arr[i], i, arr)) {
                    return true;
                }
        ...
        this.bindFunction('every', async (arr, predicate) => {
            for (let i = 0; i < arr.length; i++) {
                if (!(await predicate(arr[i], i, arr))) {
                    return false;
                }
        ...
        this.bindFunction('sort', async (arr, compareFn) => {
            if (!compareFn)
                return [...arr].sort();
        ...
        this.bindFunction('slice', (arr, start, end) => arr.slice(start, end));
        this.bindFunction('concat', (...arrays) => [].concat(...arrays));
        this.bindFunction('join', (arr, separator) => arr.join(separator));
        this.bindFunction('includes', (arr, item) => arr.includes(item));
        this.bindFunction('indexOf', (arr, item) => {
        ...
        // String functions
        this.bindFunction('toUpperCase', (str) => str.toUpperCase());
        this.bindFunction('toLowerCase', (str) => str.toLowerCase());
        this.bindFunction('trim', (str) => str.trim());
        this.bindFunction('split', (str, separator) => separator !== undefined ? str.split(separator) : str.split(''));
        this.bindFunction('replace', (str, search, replacement) => str.replace(search, replacement));
        this.bindFunction('substring', (str, start, end) => str.substring(start, end));
        this.bindFunction('charAt', (str, index) => str.charAt(index));
        this.bindFunction('charCodeAt', (str, index) => str.charCodeAt(index));
        this.bindFunction('startsWith', (str, search) => str.startsWith(search));
        this.bindFunction('endsWith', (str, search) => str.endsWith(search));
        this.bindFunction('repeat', (str, count) => str.repeat(count));
        this.bindFunction('padStart', (str, length, fill) => str.padStart(length, fill));
        this.bindFunction('padEnd', (str, length, fill) => str.padEnd(length, fill));
        ...
        // Type checking
        this.bindFunction('typeof', (val) => typeof val);
        ...
        // Async
        this.bindFunction('wait', (ms) => new Promise((resolve) => setTimeout(resolve, ms)));
    ...
    async execute(code, context, options) {
        ...
        try {
            ...
            if (parser.results.length === 0) {
                throw new errors_1.WangError('No parse found', { type: 'ParseError' });
            }
            ...
            if (error instanceof errors_1.WangError) {
                throw error;
            }
            const errorMessage = error instanceof Error ? error.message : JSON.stringify(error);
            throw new errors_1.WangError(`Parse error: ${errorMessage}`, { type: 'ParseError' }, error instanceof Error ? error : undefined);
        }
    }
    createSyncFunction(node) {
        ...
        switch (node.type) {
            ...
            case 'MemberExpression':
                ...
                if (!node.optional && obj == null) {
                    ...
                    const error = new errors_1.TypeMismatchError('object', obj, `accessing property '${propName}' of '${objName}'`);
                    this.enhanceErrorWithContext(error, node);
                    throw error;
                }
            ...
            case 'BinaryExpression':
                ...
                switch (node.operator) {
                    ...
                    default:
                        throw new errors_1.WangError(`Unknown binary operator: ${node.operator}`, {
                            type: 'RuntimeError',
                            suggestions: [
            ...
            case 'UnaryExpression':
                ...
                switch (node.operator) {
                    ...
                    case 'typeof':
                        return typeof arg;
                    default:
                        throw new errors_1.WangError(`Unknown unary operator: ${node.operator}`, {
                            type: 'RuntimeError',
                            suggestions: [
            ...
            case 'CallExpression':
                ...
                if (typeof callee !== 'function') {
                    const error = new errors_1.TypeMismatchError('function', callee, "calling 'member expression'");
    ...
    async evaluateNode(node) {
        ...
        switch (node.type) {
            ...
            case 'SpreadElement':
                ...
                if (!Array.isArray(arr)) {
                    const error = new errors_1.TypeMismatchError('array', arr, 'spread operator');
                    this.enhanceErrorWithContext(error, node);
                    throw error;
                }
                return arr;
            default:
                throw new errors_1.WangError(`Unknown node type: ${node.type}`, { type: 'RuntimeError' });
        }
    }
    async evaluateProgram(node) {
    ...
    processContinuations(statements) {
        const result = [];
    ...
    assignPattern(pattern, value, kind) {
        if (pattern.type === 'Identifier') {
            if (kind === 'var') {
            ...
            if (value == null) {
                ...
                const error = new errors_1.TypeMismatchError(patternType, value, `destructuring assignment`);
    ...
    canExpressionExecuteSynchronously(expr) {
        switch (expr.type) {
            case 'Identifier':
            case 'Literal':
            case 'BooleanLiteral':
    ...
    async evaluateClassDeclaration(node) {
        ...
        const constructorMethod = node.body.body.find((m) => m.kind === 'constructor');
        ...
        const classConstructor = function (...args) {
            ...
            return (async () => {
                ...
                if (constructorMethod) {
                    ...
                    if (superClass) {
                        constructorContext.variables.set('__super__', async (...superArgs) => {
        ...
        for (const method of node.body.body) {
            if (method.type === 'MethodDefinition' && method.kind !== 'constructor') {
                ...
                if (method.kind === 'get' || method.kind === 'set') {
                    ...
                    // Regular method
                    const methodFunction = async function (...args) {
                    ...
                    if (method.static) {
                        classConstructor[methodName] = methodFunction;
                    }
                    else {
    ...
    async evaluateForStatement(node) {
        if (node.type === 'ForOfStatement') {
            ...
            if (iterable == null) {
                const error = new errors_1.TypeMismatchError('iterable', iterable, `for...of loop`);
                this.enhanceErrorWithContext(error, node);
                throw error;
            }
            if (typeof iterable[Symbol.iterator] !== 'function' && !Array.isArray(iterable)) {
                const error = new errors_1.TypeMismatchError('iterable (Array, Set, Map, String, etc.)', iterable, `for...of loop`);
                this.enhanceErrorWithContext(error, node);
                throw error;
            }
            ...
            if (obj == null) {
                const error = new errors_1.TypeMismatchError('object', obj, `for...in loop`);
                this.enhanceErrorWithContext(error, node);
                throw error;
            }
    ...
    async evaluateLabeledStatement(node) {
        try {
    ...
    async evaluateSwitchStatement(node) {
    ...
    async evaluateTryStatement(node) {
        let result;
        try {
    ...
    evaluateIdentifier(node) {
        ...
        const error = new errors_1.UndefinedVariableError(name, [...this.currentContext.variables.keys()]);
        this.enhanceErrorWithContext(error, node);
        throw error;
    }
    async evaluateCallExpression(node) {
        ...
        if (node.callee.type === 'Super') {
            ...
            if (!superConstructor) {
                const error = new errors_1.WangError('super() can only be called in a derived class constructor', {
                    type: 'RuntimeError',
                });
                this.enhanceErrorWithContext(error, node);
                throw error;
    ...
    async evaluateBinaryExpression(node) {
        ...
        switch (node.operator) {
            ...
            case 'instanceof':
                ...
                return left instanceof right;
            default:
                throw new errors_1.WangError(`Unknown binary operator: ${node.operator}`, { type: 'RuntimeError' });
        }
    }
    async evaluateUnaryExpression(node) {
        ...
        if (node.operator === '++' || node.operator === '--') {
            if (node.argument.type !== 'Identifier') {
                throw new errors_1.WangError('Prefix increment/decrement only supports identifiers', {
                    type: 'RuntimeError',
                });
            }
        ...
        switch (node.operator) {
            ...
            default:
                throw new errors_1.WangError(`Unknown unary operator: ${node.operator}`, { type: 'RuntimeError' });
        }
    }
        ...
        if (node.left.type === 'Identifier') {
            ...
            switch (node.operator) {
                ...
                case '+=':
                    ...
                    // Update in the correct context
                    let addCtx = this.currentContext;
                    while (addCtx) {
                ...
                case '-=':
                    ...
                    let subCtx = this.currentContext;
                    while (subCtx) {
                ...
                case '*=':
                    ...
                    let mulCtx = this.currentContext;
                    while (mulCtx) {
                ...
                case '/=':
                    ...
                    return newDivVal;
                default:
                    throw new errors_1.WangError(`Assignment operator ${node.operator} not implemented`, {
                        type: 'RuntimeError',
                    });
            }
        }
            ...
            const property = node.left.computed
                ...
                : node.left.property.name;
            if (object == null) {
                throw new errors_1.WangError(`Cannot set property '${property}' on null or undefined`, {
                    type: 'RuntimeError',
                });
            }
            switch (node.operator) {
                ...
                case '/=':
                    ...
                    return object[property];
                default:
                    throw new errors_1.WangError(`Assignment operator ${node.operator} not implemented for member expressions`, { type: 'RuntimeError' });
            }
        }
        throw new errors_1.WangError('Complex assignment patterns not yet implemented', {
            type: 'RuntimeError',
        });
    }
    async evaluateUpdateExpression(node) {
        if (node.argument.type === 'Identifier') {
            ...
            if (object == null) {
                const objName = node.argument.object.type === 'Identifier' ? node.argument.object.name : 'expression';
                throw new errors_1.TypeMismatchError('object', object, `updating property '${property}' of '${objName}'`);
            }
        ...
        throw new errors_1.WangError('Update expression only supports identifiers and member expressions', {
            type: 'RuntimeError',
        });
    }
    async evaluateConditionalExpression(node) {
        const test = await this.evaluateNode(node.test);
        return test
        ...
        switch (methodName) {
            case 'filter':
                return async (predicate) => {
                    const filterFn = interpreter.currentContext.functions.get('filter') ||
                        interpreter.globalContext.functions.get('filter');
                    return filterFn ? await filterFn(arr, predicate) : arr.filter(predicate);
                };
            case 'map':
                return async (mapper) => {
                    const mapFn = interpreter.currentContext.functions.get('map') ||
            ...
            case 'reduce':
                return async (reducer, initial) => {
                    const reduceFn = interpreter.currentContext.functions.get('reduce') ||
            ...
            case 'find':
                return async (predicate) => {
                    const findFn = interpreter.currentContext.functions.get('find') ||
            ...
            case 'some':
                return async (predicate) => {
                    const someFn = interpreter.currentContext.functions.get('some') ||
                        interpreter.globalContext.functions.get('some');
                    return someFn ? await someFn(arr, predicate) : arr.some(predicate);
                };
            case 'every':
                return async (predicate) => {
                    const everyFn = interpreter.currentContext.functions.get('every') ||
            ...
            case 'sort':
                return (compareFn) => {
                    const sortFn = interpreter.currentContext.functions.get('sort') ||
            ...
            case 'reverse':
                return () => {
                    const reverseFn = interpreter.currentContext.functions.get('reverse') ||
    ...
    async evaluateNewExpression(node) {
        ...
        if (typeof constructor !== 'function') {
            const error = new errors_1.TypeMismatchError('constructor', constructor, 'new expression');
            this.enhanceErrorWithContext(error, node);
            throw error;
        }
        const args = [];
    ...
    async evaluateArrayExpression(node) {
        const result = [];
    ...
    async evaluateObjectExpression(node) {
        const result = {};
        for (const prop of node.properties) {
            if (prop.type === 'SpreadElement') {
    ...
    async interpolateTemplate(template) {
    ...
    async evaluateTemplateExpression(expression) {
