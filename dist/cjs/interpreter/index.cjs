...
class WangInterpreter {
    ...
    async evaluateNode(node) {
        ...
        switch (node.type) {
            ...
            case 'SpreadElement':
                ...
                if (!Array.isArray(arr)) {
                    throw new errors_1.TypeMismatchError('array', arr, 'spread operator');
                }
                return arr;
            default:
    ...
    evaluateIdentifier(node) {
        ...
        throw new errors_1.UndefinedVariableError(name, [...this.currentContext.variables.keys()]);
    }
    async evaluateCallExpression(node) {
        ...
        if (node.callee.type === 'Super') {
            ...
            if (!superConstructor) {
                throw new errors_1.WangError('super() can only be called in a derived class constructor', {
                    type: 'RuntimeError',
                });
            }
            const args = [];
        ...
        if (typeof callee !== 'function') {
            throw new errors_1.TypeMismatchError('function', callee, 'call expression');
        }
        const args = [];
        for (const arg of node.arguments) {
            if (arg.type === 'SpreadElement') {
    ...
    async evaluateUpdateExpression(node) {
        if (node.argument.type === 'Identifier') {
            ...
            if (object == null) {
                throw new errors_1.WangError(`Cannot update property '${property}' on null or undefined`, {
                    type: 'RuntimeError',
                });
            }
    ...
    async evaluateNewExpression(node) {
        ...
        if (typeof constructor !== 'function') {
            throw new errors_1.TypeMismatchError('constructor', constructor, 'new expression');
        }
        const args = [];
        for (const arg of node.arguments) {
            args.push(await this.evaluateNode(arg));
