...
export class WangInterpreter {
   ...
   */
  private findSimilarNames(target: string, available: string[]): string[] {
    ...
    for (const name of available) {
      ...
      if (name_lower === target_lower) continue;
  ...
  private canStatementExecuteSynchronouslyForCallback(stmt: any): boolean {
    switch (stmt.type) {
      case 'ExpressionStatement':
      ...
      case 'IfStatement':
        return (
          this.canExpressionExecuteSynchronouslyForCallback(stmt.test) &&
      ...
      case 'BlockStatement':
        return stmt.body.every((s: any) => this.canStatementExecuteSynchronouslyForCallback(s));
      default:
  ...
  private canExpressionExecuteSynchronouslyForCallback(expr: any): boolean {
    switch (expr.type) {
      case 'Identifier':
      case 'Literal':
      case 'BooleanLiteral':
      ...
      case 'LogicalExpression':
        return (
          this.canExpressionExecuteSynchronouslyForCallback(expr.left) &&
      ...
      case 'MemberExpression':
        return (
          this.canExpressionExecuteSynchronouslyForCallback(expr.object) &&
      ...
      case 'ArrayExpression':
        return expr.elements.every((elem: any) => {
      ...
      case 'ConditionalExpression':
        return (
          this.canExpressionExecuteSynchronouslyForCallback(expr.test) &&
  ...
  private canExpressionExecuteSynchronously(expr: any): boolean {
    switch (expr.type) {
      ...
      case 'ArrayExpression':
        ...
        return expr.elements.every((elem: any) => {
          if (elem === null) return true; // Holes in arrays
          if (elem.type === 'SpreadElement') {
      ...
      case 'ConditionalExpression':
        ...
        return (
          this.canExpressionExecuteSynchronously(expr.test) &&
   ...
   */
  private isNativeConstructor(fn: Function): boolean {
