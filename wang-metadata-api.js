...
export class WangMetadata {
  ...
  recordNode(node, depth = 0) {
    ...
    this.compilation.nodeTypes.set(
      node.type,
      (this.compilation.nodeTypes.get(node.type) || 0) + 1
    )
  ...
  recordVariableAccess(name, type, value = undefined) {
    if (type === 'read') {
      this.execution.variableReads.set(
        name,
        (this.execution.variableReads.get(name) || 0) + 1
      )
    } else if (type === 'write') {
      this.execution.variableWrites.set(
        name,
        (this.execution.variableWrites.get(name) || 0) + 1
      )
  ...
  getInterpretationSummary() {
    return {
      ...
      totalDependencies: Array.from(this.interpretation.moduleDependencies.values())
    ...
    return {
      ...
      interpretation: {
        ...
        moduleDependencies: Array.from(this.interpretation.moduleDependencies.entries())
          .map(([module, deps]) => ({ module, dependencies: Array.from(deps) }))
      },
      execution: {
        ...this.execution,
        ...
        variableTypes: Array.from(this.execution.variableTypes.entries())
          .map(([name, types]) => ({ name, types: Array.from(types) }))
      },
      runtime: {
        ...this.runtime,
        liveVariables: Array.from(this.runtime.liveVariables.entries())
          .map(([name, value]) => ({ name, type: typeof value, value: this._serializeValue(value) })),
